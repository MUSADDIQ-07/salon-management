...
function loadFromStorage() {
    try {
        ...
        if (savedData) {
            ...
            saveToStorage(); // Save sample data for future
            console.log('No saved data found, using sample data');
            return false;
        }
    } catch (error) {
        console.error('Error loading from localStorage:', error);
        subscribers = [...sampleData]; // Fallback to sample data
    ...
    reader.onload = function(e) {
        try {
            ...
            if (backupData.subscribers && Array.isArray(backupData.subscribers)) {
                ...
                showMessage(`Backup imported successfully! ${subscribers.length} subscribers loaded.`, 'success');
            } else {
...
function getStorageInfo() {
    try {
        ...
        return {
            ...
            lastUpdated: JSON.parse(data || '{}').timestamp || 'Never'
        };
    } catch (error) {
        return {
            subscriberCount: 0,
...
function initTabNavigation() {
    ...
    console.log('Initializing tab navigation...', navBtns.length, 'buttons found');
...
// Add Subscriber with localStorage
function initAddSubscriber() {
    ...
    form.addEventListener('submit', (e) => {
        ...
        if (saveToStorage()) {
            ...
            showMessage('Subscriber added and saved successfully!', 'success');
            updateDashboard();
        } else {
            // Remove from array if save failed
            subscribers.pop();
            showMessage('Failed to save subscriber data', 'error');
        }
    });
}

// Edit Subscriber with localStorage
function initEditSubscriber() {
    ...
    form.addEventListener('submit', (e) => {
        ...
        if (subscriberIndex !== -1) {
            const originalSubscriber = { ...subscribers[subscriberIndex] };
            ...
            if (saveToStorage()) {
                ...
                updateSubscribersTable();
                updateDashboard();
                showMessage('Subscriber updated and saved successfully!', 'success');
            } else {
                // Revert changes if save failed
                subscribers[subscriberIndex] = originalSubscriber;
                showMessage('Failed to save subscriber changes', 'error');
            }
        }
    });
}
...
// Delete Subscriber with localStorage
    ...
    confirmYes.addEventListener('click', () => {
        if (deleteId) {
            ...
            if (saveToStorage()) {
                ...
                updateSubscribersTable();
                updateDashboard();
                showMessage('Subscriber deleted and saved successfully!', 'success');
            } else {
                // Revert if save failed
                subscribers = originalSubscribers;
                showMessage('Failed to delete subscriber', 'error');
            }
...
function updateStats() {
    ...
    if (totalSubEl) totalSubEl.textContent = totalSubscribers;
    if (activeSubEl) activeSubEl.textContent = activeSubscriptions;
    if (expiringSubEl) expiringSubEl.textContent = expiringSubscriptions;
    ...
    if (storageInfoContainer) {
        storageInfoContainer.innerHTML = `
            <div class="storage-item">
                ...
                <strong>${storageInfo.subscriberCount}</strong>
            </div>
            <div class="storage-item">
                <span>Storage Used:</span>
                <strong>${storageInfo.storageSize} KB</strong>
            </div>
            <div class="storage-item">
                <span>Last Updated:</span>
                <strong>${formatDate(storageInfo.lastUpdated)}</strong>
            </div>
        `;
    }
}
...
// Clear Data with localStorage
function initClearData() {
    const clearBtns = document.querySelectorAll('#clearDataBtn');
    clearBtns.forEach(btn => {
        if (btn) {
            btn.addEventListener('click', (e) => {
                ...
                if (confirmMessage) {
                    confirmMessage.textContent = 'Are you sure you want to clear all data? This action cannot be undone.';
                }
...
function init() {
    ...
    window.deleteSubscriber = window.deleteSubscriber || function() {}; // Ensure it's available
    
    console.log(`Application initialized with ${subscribers.length} subscribers loaded from storage`);
}
